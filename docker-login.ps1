...
$pyCode += '    try:' + "`n"
$pyCode += '        await asyncio.sleep(3)' + "`n"
$pyCode += '        # Use JavaScript to fill (bypasses visibility check)' + "`n"
$pyCode += '        await page.evaluate(f"""() => {{' + "`n"
$pyCode += '            const inputs = document.querySelectorAll("input[autocomplete=username], input[type=email]");' + "`n"
$pyCode += '            for (const el of inputs) {{ el.value = "{email}"; el.dispatchEvent(new Event("input", {{bubbles:true}})); }}' + "`n"
$pyCode += '            const pass_inputs = document.querySelectorAll("input[type=password]");' + "`n"
$pyCode += '            for (const el of pass_inputs) {{ el.value = "{password}"; el.dispatchEvent(new Event("input", {{bubbles:true}})); }}' + "`n"
$pyCode += '        }}""")' + "`n"
$pyCode += '        await asyncio.sleep(2)' + "`n"
$pyCode += '        # Click submit' + "`n"
$pyCode += '        btn = page.locator("button[type=\"submit\"]").first' + "`n"
$pyCode += '        if await btn.is_visible():' + "`n"
$pyCode += '            await btn.click()' + "`n"
$pyCode += '        else:' + "`n"
$pyCode += '            await page.keyboard.press("Enter")' + "`n"
